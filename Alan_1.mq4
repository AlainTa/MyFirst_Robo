...
double mop, mcl, wop, wcl, dop, dcl, h1op, h1cl, h4op, h4cl;
...
int start() {
   ...
   mop = iOpen(NULL,PERIOD_MN1,0);
   mcl = iClose(NULL,PERIOD_MN1,0);
   
   wop = iOpen(NULL,PERIOD_W1,0);
   wcl = iClose(NULL,PERIOD_W1,0);
   
   dop = iOpen(NULL,PERIOD_D1,1);
   dcl = iClose(NULL,PERIOD_D1,1);
   
   h4op = iOpen(NULL,PERIOD_H4,1);
   h4cl = iClose(NULL,PERIOD_H4,1);
   
   h1op = iOpen(NULL,PERIOD_H1,1);
   h1cl = iClose(NULL,PERIOD_H1,1);
      ...
      if(OrderType() <= OP_SELL &&  OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumber) {
         ...
         if(OrderType() == OP_BUY) {
            ...
            */
            ...
            if(dop >= dcl) Order = SIGNAL_CLOSEBUY; // Rule to EXIT a Long trade
            ...
            if (dop <= dcl) Order = SIGNAL_CLOSESELL; // Rule to EXIT a Short trade
   ...
   */
   ...
   if (dop <= dcl && h4op <= h4cl && h1op <= h1cl) Order = SIGNAL_BUY; // Rule to ENTER a Long trade

   if (dop >= dcl && h4op >= h4cl && h1op >= h1cl) Order = SIGNAL_SELL; // Rule to ENTER a Short trade
